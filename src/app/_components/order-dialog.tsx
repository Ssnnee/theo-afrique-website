...
export default function OrderDialog({
	...
	useEffect(() => {
		...
		if (selectedSizes.length > quantity) {
			errors.push(
				`Vous ne pouvez sélectionner que ${quantity} taille(s) maximum`,
			);
		}

		if (selectedColors.length > quantity) {
			errors.push(
				`Vous ne pouvez sélectionner que ${quantity} couleur(s) maximum`,
			);
		}
				...
				<Form {...form}>
					<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
						...
						<div className="p-2 text-sm">
							Quantité: {quantity} | Tailles: {summary.sizesCount} | Couleurs:{" "}
							{summary.colorsCount}
						</div>
											...
											return (
												...
												>
													<Checkbox
														...
														disabled={isDisabled}
														onCheckedChange={() =>
															toggleMultiSelect("selectedSizes", size)
														}
													/>
											...
											return (
												...
												>
													<Checkbox
														...
														disabled={isDisabled}
														onCheckedChange={() =>
															toggleMultiSelect("selectedColors", color)
														}
													/>
													<span className={isSelected ? "font-medium" : ""}>
														{color}
									...
									render={({ field }) => (
										<FormItem>
											...
											<FormControl>
												<Input
													{...field}
													placeholder="Ex: M, L, XL ou mesures spécifiques"
												/>
											</FormControl>
											<FormMessage />
										</FormItem>
									...
									render={({ field }) => (
										<FormItem>
											...
											<FormControl>
												<Input
													{...field}
													placeholder="Ex: 32, 34, 36 ou mesures spécifiques"
												/>
											</FormControl>
											<FormDescription>
